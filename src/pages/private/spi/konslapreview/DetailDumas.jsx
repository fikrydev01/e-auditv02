...
import { FILTER_ALUR_DUMAS_SPI, FILTER_ALURKONSLAP_SPI, FORMAT_DATE_HOUR_IND, FORMAT_DATE_IND } from "../../../../constant/data";
import ReviewKonsultasi from "./ReviewKonsultasi";
import SPIChat from "../../SPIChat";
import SuratTugas from "./SuratTugas";
import ReviewDumas from "./ReviewDumas";
              ...
              <div className="section-body w-full lg:w-4/6">
                <div className="flex flex-col gap-4">
                  {[
                    ...
                    {
                      label: "Status",
                      value: FILTER_ALUR_DUMAS_SPI(data?.status_id),
                    },
                  ].map((item, idx) => (
                    <div
                      key={idx}
  ...
  {Array.isArray(data?.dokumen) && data.dokumen.length > 0 ? (
    data.dokumen.map((item, index) => (
      ...
      >
        ...
        >
          {item.uuid || item.path?.split("/").pop() || `Dokumen ${index + 1}`}
        </a>
      </li>
    ))
  ) : (
  ...
  >
    ...
    Surat Tugas
            ...
            {mmod === "surtug" && <SuratTugas data={data} />}
                                {mmod === "detail" && <ReviewDumas data={data} reload={reload} />}
