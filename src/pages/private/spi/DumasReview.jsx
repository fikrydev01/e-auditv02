...
import Detail from "./konslapreview/Detail";
import DetailDumas from "./konslapreview/DetailDumas";
...
const DumasReview = () => {
  ...
  const konsultasiDatas = datas?.filter((r) =>
    r?.jenis?.toLowerCase().includes("konsultasi")
  );
  const laporanData = datas?.filter((r) =>
    r?.jenis?.toLowerCase().includes("laporan")
  );

  const [mod, setMod] = useState("konsultasi");
  ...
  return (
    <section className="section-container">
      <Breadcrumbs title="Dumas Review" />
      {dtp ? 
              ...
              {filterData &&
                filterData.map((r, i) => (
                  ...
                  >
                    ...
                    <td>{FILTER_ALURKONSLAP_SPI(r.status_id)}</td>
                  </tr>
                ))}
            </tbody>
          </table>
