...
const KkCash = ({ data }) => {
  ...
  // PDTT. KHUSUS UNIT NYA DIAMBIL DARI DATA AUDIT

  const unit = data?.kode_unit;
  ...
  const getDatas = () => {
    fetchData(
      `/spi/audit_kas?audit_id=${data?.id}&kode_unit=${unit}&periode_audit=${data?.periode_audit}`,
      {
